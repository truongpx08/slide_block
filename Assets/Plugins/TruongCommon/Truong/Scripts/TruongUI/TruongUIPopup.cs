...
public abstract class TruongUIPopup : TruongUIButton
{
    ...
    {
        title = transform.Find(TruongConstants.POPUP).Find(TruongConstants.TITLE);
    }

    private void LoadPopup()
    {
        popup = transform.Find(TruongConstants.POPUP);
    }
    ...
    {
        if (!Popup || !Application.isPlaying) return;
        ...
        SetAnimation_ShowPopup(Popup, obj.gameObject, action, id);
    }
    ...
    public void HidePopup(Action action = null)
    {
        ...
        SetAnimation_HidePopup(Popup, obj.gameObject, action);
    }
